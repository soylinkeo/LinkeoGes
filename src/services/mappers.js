const legacyMappers = {
  ...
  nfcToFront: (c) => ({
    id: c.id,
    chipUid: c.uid || c.chipUid || c.payload?.chipUid || '',
    batch: c.batch || c.payload?.batch || '',
    model: c.model || c.payload?.model || 'Tarjeta NFC',
    category: c.category || c.payload?.category || 'Individual',
    reviewUrl: c.url || c.reviewUrl || c.payload?.reviewUrl || '',
    placeId: c.placeId || c.payload?.placeId || '',
    qrControlUrl: c.qrControlUrl || c.payload?.qrControlUrl || '',
    status: (c.status && c.status !== 'virgen') ? c.status : (c.payload?.status || 'Activa'),
    businessName: c.assigned_to || c.businessName || c.payload?.businessName || '',
    district: c.district || c.payload?.district || '',
    address: c.address || c.payload?.address || '',
    contactName: c.contactName || c.payload?.contactName || '',
    contactPhone: c.contactPhone || c.payload?.contactPhone || '',
    notes: c.notes || c.payload?.notes || '',
    history: Array.isArray(c.history) ? c.history : (Array.isArray(c.payload?.history) ? c.payload.history : []),
    renewalDate: c.renewalDate || c.payload?.renewalDate || '',
    assignedDate: c.assignedDate || c.payload?.assignedDate || '',
    leadId: c.leadId || c.payload?.leadId || null,
    productId: c.productId || c.payload?.productId || null,
    productSku: c.productSku || c.payload?.productSku || null,
